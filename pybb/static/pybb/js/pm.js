function switchcb(cb, state) {
    if (state) {
        $(cb).attr('checked', 'checked').parent().parent().addClass('chosen');
    } else {
        $(cb).removeAttr('checked').parent().parent().removeClass('chosen');
    }
...
$(document).ready(function() {
    ...
    $('.selectcb').each(function(){
        switchcb(this, this.checked)
    }).click(function(){
        switchcb(this, this.checked)
    });

    $('#actionselect').val('').change(function(){
        multiaction(this.value)
    });
});
